angular.module('todos.controller',['ngRoute'])
	   .config(['$routeProvider',function($routeProvider){
		   $routeProvider
		   			.when('/:status',{templateUrl:'todoId',controller:'todos.controller'})
					.otherwise({//默认的情况，比如根路径就会走这里
						redirectTo:'/all'
					});   
	   }])	
       .controller('todos.controller',function($scope,$routeParams){
		   	console.log($routeParams.status);
			switch($routeParams.status){
				case 'all':
					$scope.isCompleted1 = undefined;
					break;
				case 'completed':
